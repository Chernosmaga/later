...
import com.example.later.item.model.ItemCountByUser;
import com.example.later.item.model.ItemInfo;
...
public interface ItemRepository extends JpaRepository<Item, Long>, QuerydslPredicateExecutor<Item> {
    ...
    @Query("select new com.example.later.item.model.ItemCountByUser(it.user.id, count(it.id))" +
            "from Item as it "+
            "where it.url like ?1 "+
            "group by it.user.id "+
